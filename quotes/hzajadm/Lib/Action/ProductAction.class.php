...
class ProductAction extends CommonAction{
	...
	public function index(){
		...
		$this->product=$db->field('id,pid,name,sort,thumb,featured,xx,yy,zz')->order('sort asc,id desc')->limit($page->firstRow.','.$page->listRows)->select();
	...
	public function mod(){
		...
		$product=M('Product')->field('id,pid,sort,name,property1,property2,property3,property4,property5,property6,property7,property8,property9,url,title,keywords,description,thumb,photo,contents,contents1,contents2,contents3,featured,xx,yy,zz')->find($this->_get('id','intval'));
	...
	public function savepro(){
		...
		if($data=$db->create()){
			...
			if($db->data($data)->add()){
				...
				$this->success('信息添加成功',U('Product/index'));
	...
	public function updatepro(){
		...
		$data['bid']=$this->getbigid($data['pid']);	
			...
			if($db->data($data)->save()){
				...
				$this->success('更新信息成功',U('Product/index'));
