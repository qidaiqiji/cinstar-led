...
class PhotoAction extends CommonAction{
	...
	public function index(){
		...
		$this->photo=$db->field('id,pid,name,sort,thumb,featured,toped,hotted,zuozhe,zuozhe2,title')->order('sort asc,id desc')->limit($page->firstRow.','.$page->listRows)->select();
	...
	public function mod(){
		...
		$photolist=M('Photo')->field('id,pid,sort,name,url,title,keywords,description,thumb,photo,contents,featured,toped,hotted,property1,property2,zuozhe,zuozhe2')->find($this->_get('id','intval'));
	...
	public function savephoto(){
		...
		if($data=$db->create()){
			...
			if($db->data($data)->add()){
				...
				$this->success('信息添加成功',U('Photo/index'));
		  ...
		  $photo=rtrim($photo,",");
			...
			if(M('Photo')->where('id='.$id)->setField($field,$photo)){
				...
				if(delimg('../Uploads/download/'.$name)){
					...
					$this->success('数据删除成功，但找不到要删除的信息',U('mod',array('id'=>$id)));
	...
	public function del(){
		...
		if($db->where('id ='.$id)->delete()){
			...
			delimg('../Uploads/download/'.$thumb);

			delallimg('../Uploads/download/',$photo);
	...
	public function seach(){
		...
		$this->getSearch('photo','title','photo');
