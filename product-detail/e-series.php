<!DOCTYPE html>
<html lang="en">
<head>
    <?php include('../inc/meta.php'); ?>
    <?php include('../inc/rem.php'); ?>
    <link rel="stylesheet" href="../css/detail.css">
    <title>LED Rental Panel | Indoor Outdoor LED Panel Screen | LED Screen Rental | Cinstar-led</title>
    <meta name="description" content="Featuring wide pixel pitches from indoor 2.6mm, 2.9mm, 3.9mm to outdoor 3.9mm, 4.8mm and 5.6mm, Cinstar E SERIES rental LED panel has two panel sizes 500x500mm and 500x1000mm to meet any of your LED rental event productions." />
    <meta name="keywords" content="Rental LED screen, Indoor LED Rental Panel, Outdoor LED Rental Panel, Rental events panel, indoor LED panel, outdoor led panel, led panel rental, led screen rental" />
</head>
<body class="page-index .menu-open">
    <!-- 头部开始 -->
    <?php include('../inc/top.php'); ?>
    <!-- 头部结束 -->
    <div class="container">
        <div class="detail-title tl-bg bg">
            <div class="title">
                <span>E SERIES</span>
            </div>
            <div class="top-title">Event Production Rental LED Video Wall Panel</div>
        </div>
  
        <!-- 产品参数 -->
        <div class="specification">
            <div class="spec-detail">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td colspan="2">Specifications</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm):
                            </td>
                            <td>
                            2.6 / 2.9 / 3.9
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nits):
                            </td>
                            <td>
                            up to 6000
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Refresh Rate(Hz):    
                            </td>
                            <td>
                            1920-3840
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Application: 
                            </td>
                            <td>
                            Indoor & Outdoor Event Production
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div class="key-feature">
                    <div>Key Features</div>
                    <ul>
                        <li>Two panel sizes for option</li>
                        <li>Modular design for easy maintenance </li>
                        <li>Versatile creative installation</li>
                        <li>Seamless screen connection</li>
                        <li>Practical corner protection design</li>
                        <li>Suspension and floor stacking</li>
                    </ul>
                </div>
            </div>
            <div class="product-img small-long" style="width: 30%">
                <img src="../images/products/Rental-LED/E-series-1.jpg" alt="E SERIES"/>
            </div>
        </div>
        <!-- 产品详情 -->
        <div class="product-detail">
            <div class="detail">
                <div class="img-part half">
                    <img src="../images/products/Rental-LED/E-series-2.jpg" alt="E SERIES"/>
                </div>
                <div class="desc-part half">
                    <h2 class="title">High Profile Robust Mechanical Design </h2>
                    <p>
                    Cinstar E series indoor and outdoor rental LED video wall is featured with high precision die-cast aluminum for perfect seamless connection and splicing. Fast locks enable set-ups and dismantle extremely quick and easy. The eye-catching and robust panel design make it reliable for any of your high profile rental events.
                    </p>
                </div>
            </div>
            <hr class="under-line"/>
            <div class="detail">
                <div class="desc-part half">
                    <h2 class="title">Modular Design For Easy Maintenance </h2>
                    <p>
                    Cinstar E series rental LED video wall is featured with modular design, all the modules and power box can be maintained from both front and rear sides. The maintenance is extremely quick and easy, brings no surprise to your events. These user friendly designs can greatly ease your maintenance work and give you peace of mind.
                    </p>
                </div>
                <div class="img-part half">
                    <img src="../images/products/Rental-LED/E-series-3.jpg" alt="E SERIES"/>
                </div>
            </div>
            <hr class="under-line"/>
            <div class="detail">
                <div class="img-part half">
                    <img src="../images/products/Rental-LED/E-series-4.jpg" alt="E SERIES"/>
                </div>
                <div class="desc-part half">
                    <h2 class="title">Versatile Creative Installation</h2>
                    <p>
                    Cinstar E series rental LED video wall is so versatile that it could be used as flat LED video wall, right angle set-up, concave or convex installation. All  these features make it easy for you to achieve any special shape and creative effect.
                    </p>
                </div>
            </div>
            <hr class="under-line"/>
            <div class="detail">
                <div class="desc-part half">
                    <h2 class="title">Seamless Screen Connection</h2>
                    <p>
                    Cinstar E series rental LED video wall is designed with positioning pins for seamless connection and anti-collision device to protect the bottom LEDs from damage. The fast side locks can make the installation and removal very quick and easy.
                    </p>
                </div>
                <div class="img-part half">
                    <img src="../images/products/Rental-LED/E-series-5.jpg" class="special" alt="E SERIES"/>
                </div>
            </div>
            <hr class="under-line"/>
            <div class="detail">
                <div class="img-part half">
                    <img src="../images/products/Rental-LED/E-series-6.jpg" alt="E SERIES"/>
                </div>
                <div class="desc-part half">
                    <h2 class="title">Practical Corner Protection Design</h2>
                    <p>
                    Corner protection flip can effectively protect the corners of panel when doing set up and transport in road case. Which can greatly minimize the dead pixels caused by collision.
                    </p>
                </div>
            </div>
            <hr class="under-line"/>
            <div class="detail">
                <div class="desc-part half">
                    <h2 class="title">Top Stability For All Events</h2>
                    <p>
                    Featured with high IP rating (IP65), Cinstar E series can be applied for any of your events, no matter it is for indoor or outdoor applications. One investment on one pitch can satisfy all your requirements, optimizing your ROI.
                    </p>
                </div>
                <div class="img-part half">
                    <img src="../images/products/Rental-LED/E-series-7.jpg" class="special" alt="E SERIES"/>
                </div>
            </div>
            <hr class="under-line"/>
            <div class="detail">
                <div class="img-part half">
                    <img src="../images/products/Rental-LED/E-series-8.jpg" alt="E SERIES"/>
                </div>
                <div class="desc-part half">
                    <h2 class="title">Two Panel Sizes Maximize Your Events Possibilities</h2>
                    <p>
                    Cinstar E series rental LED video wall has 500x500mm and 500x1000mm panel sizes for option. With main batch of 500x1000mm and a certain proportion of 500x500mm, you can do any events with very limited  budget. 
                    </p>
                </div>
            </div>
            <hr class="under-line"/>
            <div class="detail">
                <div class="desc-part half">
                    <h2 class="title">Suspension And Floor Stacking Available</h2>
                    <p>
                    Cinstar E series rental LED video wall can be installed as hanging or floor stacking to meet the various requirements of your events. More possibilities can bring you more business opportunities!
                    </p>
                </div>
                <div class="img-part half">
                    <img src="../images/products/Rental-LED/E-series-9.jpg" class="special" alt="E SERIES"/>
                </div>
            </div>
        </div>
        <!-- SERIES FEATURES -->
        <div class="features">
            <div class="sub-title">
                <span>E SERIES Features</span>
            </div>
            <div class="iconsList">
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-check"></i>
                    </div>
                    <p class="icon-desc">Two panel sizes for option</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-bars"></i>
                    </div>
                    <p class="icon-desc">Truly modular design</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe621;</i>
                    </div>
                    <p class="icon-desc">Versatile creative installation</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-tint"></i>
                    </div>
                    <p class="icon-desc">Seamless screen connection</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe600;</i>
                    </div>
                    <p class="icon-desc">Practical corner protection design</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-random"></i>
                    </div>
                    <p class="icon-desc">Suspension and floor stacking</p>
                </div>

            </div>
        </div>
        <!-- showCases -->
        <div class="show-cases">
            <div class="sub-title">
                <span>E SERIES Showcase</span>
            </div>
            <div class="img-wrap">
                <div class="caseItem">
                    <img src="../images/products/Rental-LED/E-series-10.jpg" alt="Cinstar E2.6 Indoor Rental LED Video Wall"/>
                    <a class="case-title">
                        <span class="current">Cinstar E2.6 Indoor Rental LED Video Wall</span>
                    </a>
                </div>
                <div class="caseItem">
                    <img src="../images/products/Rental-LED/E-series-11.jpg" alt="Cinstar E3.9 Outdoor Event Rental LED Video Wall"/>
                    <a class="case-title">
                        <span class="current">Cinstar E3.9 Outdoor Event Rental LED Video Wall</span>
                    </a>
                </div>
            </div>
        </div>
        <div class="table-detail">
            <div class="sub-title">
                <span>Technical Specification</span>
            </div>
            <div class="table-wrap">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td></td>
                            <td>E2.6</td>
                            <td>E2.9</td>
                            <td>E3.9</td>
                            <td>E2.9(Outdoor)</td>
                            <td>E3.9(Outdoor)</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm)
                            </td>
                            <td>
                            2.6
                            </td>
                            <td>
                            2.97
                            </td>
                            <td>
                            3.91
                            </td>
                            <td>
                            2.97
                            </td>
                            <td>
                            3.91
                            </td>
                        </tr>
                        <tr>
                            <td>
                                LED Type
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                            3 in 1 SMD
                            </td>
                            <td>
                            3 in 1 SMD
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Module Size (WxH)/(mm)
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Size (WxH)/(mm)
                            </td>
                            <td>
                                500x500 / 500x1000
                            </td>
                            <td>
                                500x500 / 500x1000
                            </td>
                            <td>
                                500x500 / 500x1000
                            </td>
                            <td>
                                500x500 / 500x1000
                            </td>
                            <td>
                                500x500 / 500x1000
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Pixel Per Panel
                            </td>
                            <td>
                            192x192 / 192x384
                            </td>
                            <td>
                            168x168 / 168x336
                            </td>
                            <td>
                            128x128 / 128x256
                            </td>
                            <td>
                            168x168 / 168x336
                            </td>
                            <td>
                            128x128 / 128x256
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Material
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Panel Weight (kg)
                            </td>
                            <td>
                            7.5 / 14
                            </td>
                            <td>
                            7.5 / 14
                            </td>
                            <td>
                            7.5 / 14
                            </td>
                            <td>
                            7.8 / 14.5
                            </td>
                            <td>
                            7.8 / 14.5
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nit)
                            </td>
                            <td>
                                > 1000
                            </td>
                            <td>
                                > 1000
                            </td>
                            <td>
                            > 1200
                            </td>
                            <td>
                            > 5500
                            </td>
                            <td>
                            > 5500
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Refresh Rate (Hz)
                            </td>
                            <td>
                            1920-3840
                            </td>
                            <td>
                            1920-3840
                            </td>
                            <td>
                            1920-3840
                            </td>
                            <td>
                            1920-3840
                            </td>
                            <td>
                            1920-3840
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Gray Scale (bit) 
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Viewing Angle (H/V) (°) 
                            </td>
                            <td>
                                160/160
                            </td>
                            <td>
                                160/160
                            </td>
                            <td>
                                160/160
                            </td>
                            <td>
                            160/140
                            </td>
                            <td>
                            160/140
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Avg. Power Consumption (W)
                            </td>
                            <td>
                                240/sqm
                            </td>
                            <td>
                            210/sqm
                            </td>
                            <td>
                            218/sqm
                            </td>
                            <td>
                            236/sqm
                            </td>
                            <td>
                            243/sqm
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Max. Power Consumption (W)
                            </td>
                            <td>
                            720/sqm
                            </td>
                            <td>
                            630/sqm
                            </td>
                            <td>
                            654/sqm
                            </td>
                            <td>
                            708/sqm
                            </td>
                            <td>
                            729/sqm
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Operating Temperature (℃)
                            </td>
                            <td>
                            ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                            ﹣20~﹢60
                            </td>
                            <td>
                            ﹣20~﹢60
                            </td>
                        </tr>
                        <tr>
                            <td>
                            IP Rating (Front/Rear)
                            </td>
                            <td>
                            IP40/IP21
                            </td>
                            <td>
                                IP40/IP21
                            </td>
                            <td>
                                IP40/IP21
                            </td>
                            <td>
                                IP65
                            </td>
                            <td>
                                IP65
                            </td>
                        </tr>
                        <tr>
                            <td>
                            LED Lifetime (H)
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Module Maintenance 
                            </td>
                            <td>
                                Front/Rear
                            </td>
                            <td>
                                Front/Rear
                            </td>
                            <td>
                                Front/Rear
                            </td>
                            <td>
                                Front/Rear
                            </td>
                            <td>
                                Front/Rear
                            </td>
                        </tr>
                        <tr>
                            <td>
                            PSU & Others Maintenance
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
        <div class="related">
            <div class="sub-title">
                <span>Related Products</span>
            </div>
            <div class="product-container">
                <div class="product-item">
                    <a class="thumb" href="../product-detail/k-series.php">
                        <img src="../images/products/Commercial-LED/K-series-1.jpg" class="shorter" style="width: 88%" alt="K SERIES"/>
                    </a>
                    <a class="list-title" href="../product-detail/k-series.php">
                        <span class="current">K SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/x-series.php">
                        <img src="../images/products/Commercial-LED/X-series-1.jpg" class="short" alt="X SERIES"/>
                    </a>
                    <a class="list-title" href="../product-detail/x-series.php">
                        <span class="current">X SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/innox-series.php">
                        <img src="../images/products/Commercial-LED/Innox-1.png" class="shorter" alt="INNOX SERIES"/>
                    </a>
                    <a class="list-title" href="../product-detail/innox-series.php">
                        <span class="current">INNOX SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/u-series.php">
                        <img src="../images/products/Commercial-LED/U-series-1.jpg" class="short" alt="u-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/u-series.php">
                        <span class="current">U SERIES</span><span class="more">View More</span>
                    </a>
                </div>
            </div>
        </div>
        <?php include('../inc/foot.php'); ?>
            <!-- 底部结束 -->
    </div>
    <?php include('../inc/js.php'); ?>
    <script>GetCurStyle("products");</script>
</body> 
</html>