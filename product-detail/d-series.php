<!DOCTYPE html>
<html lang="en">
<head>
    <?php include('../inc/meta.php'); ?>
    <?php include('../inc/rem.php'); ?>
    <link rel="canonical" href="https://www.cinstar-led.com/product-detail/d-series.php">
    <link rel="stylesheet" href="../css/detail-240913.css">
    <title>Interactive LED Dance Floor | LED Floor Screen | Creative LED Dance Floor Display | Cinstar-led</title>
    <meta name="description" content="Cinstar D series interactive led dance floor display screen can be applied to shopping mall, night club and concerts to create attractive atmosphere. We offer  wide variety of led floor screen for all your high profile and budget projects.">
    <meta name="keywords" content="interactive led display, interactive led floor screen, interactive led dance floor display, creative led dance floor screen, led floor display screen, interactive led floor, ">
</head>
<body class="page-index .menu-open">
    <!-- 头部开始 -->
    <?php include('../inc/top.php'); ?>
    <!-- 头部结束 -->
    <div class="container">
        <div class="detail-title d-bg bg">
            <div class="title">
                <span>D SERIES</span>
            </div>
            <h1 class="top-title">Interactive LED Dance Floor Solution</h1>
        </div>
        <!-- 产品参数 -->
        <div class="specification">
            <div class="spec-detail">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td colspan="2">Specifications</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm):
                            </td>
                            <td>
                            2.9 / 3.9 / 4.8  / 5.2 / 6.25
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nits):
                            </td>
                            <td>
                            up to 5500
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Refresh Rate(Hz):    
                            </td>
                            <td>
                            1920-3840
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Application: 
                            </td>
                            <td>
                            Indoor & Outdoor LED Dance Floor
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div class="key-feature">
                    <div>Key Features</div>
                    <ul>
                        <li>Two panel sizes for option</li>
                        <li>Wide range of pixel pitches</li>
                        <li>Budget and high profile solutions</li>
                        <li>Excellent load-bearing performance</li>
                        <li>Durable module mask for skid resistance</li>
                        <li>Smart interactive LED floor screen</li>
                    </ul>
                </div>
            </div>
            <div class="product-img small-long">
                <img src="https://www.cinstar-led.com/images/products/Commercial-LED/D-series-1.jpg" alt="D SERIES-1">
            </div>
        </div>
        <!-- 产品详情 -->
        <div class="product-detail">
            <div class="detail">
                <div class="img-part half">
                    <img src="https://www.cinstar-led.com/images/products/Commercial-LED/D-series-2.jpg" alt="D SERIES-2">
                </div>
                <div class="desc-part half">
                    <h2 class="title">500x500mm & 500x1000mm Panels For Option</h2>
                    <p>
                    Cinstar D series interactive LED dance floor screen has two panel sizes for your choose. You can greatly save budget with 500x1000mm and can do any desired sizes by applying extra 500x500mm panels.
                    </p>
                </div>
            </div>
            <hr class="under-line">
            <div class="detail">
                <div class="desc-part half">
                    <h2 class="title">Available Budget And High Profile Solutions</h2>
                    <p>
                    Cinstar D series creative LED dance floor screen offers wide range of options for your projects from high profile to budget ones with varied specifications of steel or die-cast aluminum panel, Radar interactive or smart interactive with built-in IC. We are capable of supplying a turn key solution for your projects. 
                    </p>
                </div>
                <div class="img-part half">
                    <img src="https://www.cinstar-led.com/images/products/Commercial-LED/D-series-3.jpg" alt="D SERIES-3">
                </div>
            </div>
            <hr class="under-line">
            <div class="detail">
                <div class="img-part half">
                    <img src="https://www.cinstar-led.com/images/products/Commercial-LED/D-series-4.jpg" alt="D SERIES-4">
                </div>
                <div class="desc-part half">
                    <h2 class="title">Excellent Load-Bearing Performance</h2>
                    <p>
                    Featured with robust panel frame, point-to-face contact and support technology, the weight capacity of Cinstar D series LED dance floor is more than 3000kg/sqm, which makes it a reliable floor video solution for stage events, TV studio and Car show etc. 
                    </p>
                </div>
            </div>
            <hr class="under-line">
            <div class="detail">
                <div class="desc-part half">
                    <h2 class="title">Durable Module Mask For Skid Resistance</h2>
                    <p>
                    Cinstar D series LED floor screen adopts special high quality durable module mask to make sure the screen surface is non-slip for people to walk and dancing on it. Its great durability can also protect the LEDs from damage.
                    </p>
                </div>
                <div class="img-part half">
                    <img src="https://www.cinstar-led.com/images/products/Commercial-LED/D-series-5.jpg" alt="D SERIES-5">
                </div>
            </div>
        </div>
        <!-- SERIES FEATURES -->
        <div class="features">
            <div class="sub-title">
                <span>D SERIES Features</span>
            </div>
            <div class="iconsList">
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-check"></i>
                    </div>
                    <p class="icon-desc">Two panel sizes for option</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-bars"></i>
                    </div>
                    <p class="icon-desc">Wide range of pixel pitches</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe621;</i>
                    </div>
                    <p class="icon-desc">Budget and high profile solutions</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-tint"></i>
                    </div>
                    <p class="icon-desc">Excellent load-bearing performance</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe600;</i>
                    </div>
                    <p class="icon-desc">Durable module mask for skid resistance</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-random"></i>
                    </div>
                    <p class="icon-desc">Smart interactive LED floor screen</p>
                </div>

            </div>
        </div>
        <!-- showCases -->
        <div class="show-cases">
            <div class="sub-title">
                <span>D SERIES Showcase</span>
            </div>
            <div class="img-wrap">
                <div class="caseItem">
                    <img src="https://www.cinstar-led.com/images/products/Commercial-LED/D-series-6.jpg" alt="Cinstar D5 Stage LED Dance Floor Display">
                    <a class="case-title">
                        <span class="current">Cinstar D5 Stage LED Dance Floor Display</span>
                    </a>
                </div>
                <div class="caseItem">
                    <img src="https://www.cinstar-led.com/images/products/Commercial-LED/D-series-7.jpg" alt="Cinstar D6 LED Dance Floor Screen">
                    <a class="case-title">
                        <span class="current">Cinstar D6 LED Dance Floor Screen</span>
                    </a>
                </div>
            </div>
        </div>
        <div class="table-detail">
            <div class="sub-title">
                <span>Technical Specification</span>
            </div>
            <div class="table-wrap">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td></td>
                            <td>D2.9</td>
                            <td>D3.9</td>
                            <td>D4.8</td>
                            <td>D6.2</td>
                            <td>D3.9 (Outdoor)</td>
                            <td>D5.2 (Outdoor)</td>
                            <td>D6.2 (Outdoor)</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm)
                            </td>
                            <td>
                            2.97
                            </td>
                            <td>
                            3.91
                            </td>
                            <td>
                            4.81
                            </td>
                            <td>
                            6.25
                            </td>
                            <td>
                            3.91
                            </td>
                            <td>
                            5.2
                            </td>
                            <td>
                            6.25
                            </td>
                        </tr>
                        <tr>
                            <td>
                                LED Type
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Module Size (mm)
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Panel Size (WxH)/(mm)
                            </td>
                            <td>
                            500x500 / 500x1000
                            </td>
                            <td>
                            500x500 / 500x1000
                            </td>
                            <td>
                            500x500 / 500x1000
                            </td>
                            <td>
                            500x500 / 500x1000
                            </td>
                            <td>
                            500x500 / 500x1000
                            </td>
                            <td>
                            500x500 / 500x1000
                            </td>
                            <td>
                            500x500 / 500x1000
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Pixel Per Panel
                            </td>
                            <td>
                            168x168 / 168x336
                            </td>
                            <td>
                            128x128 / 128x256
                            </td>
                            <td>
                            104x104 / 104x208
                            </td>
                            <td>
                            80x80 / 80x160
                            </td>
                            <td>
                            128x128 / 128x256
                            </td>
                            <td>
                            96x96 / 96x192
                            </td>
                            <td>
                            80x80 / 80x160
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Panel Material
                            </td>
                            <td>
                            Steel / Die-cast Aluminum
                            </td>
                            <td>
                            Steel / Die-cast Aluminum
                            </td>
                            <td>
                            Steel / Die-cast Aluminum
                            </td>
                            <td>
                            Steel / Die-cast Aluminum
                            </td>
                            <td>
                            Steel / Die-cast Aluminum
                            </td>
                            <td>
                            Steel / Die-cast Aluminum
                            </td>
                            <td>
                            Steel / Die-cast Aluminum
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Brightness (nit)
                            </td>
                            <td>
                                > 1000
                            </td>
                            <td>
                                > 1000
                            </td>
                            <td>
                                > 1000
                            </td>
                            <td>
                            > 1000
                            </td>
                            <td>
                                > 5500
                            </td>
                            <td>
                                > 5500
                            </td>
                            <td>
                            > 5500
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Refresh Rate (Hz)
                            </td>
                            <td>
                                < 3840
                            </td>
                            <td>
                                < 3840
                            </td>
                            <td>
                                < 3840
                            </td>
                            <td>
                                < 3840
                            </td>
                            <td>
                                < 3840
                            </td>
                            <td>
                                < 3840
                            </td>
                            <td>
                                < 3840
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Gray Scale (bit) 
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Viewing Angle (H/V) (°) 
                            </td>
                            <td>
                                140/140
                            </td>
                            <td>
                                140/140
                            </td>
                            <td>
                                140/140
                            </td>
                            <td>
                            140 /140
                            </td>
                            <td>
                                140/140
                            </td>
                            <td>
                                140/140
                            </td>
                            <td>
                            140 /140
                            </td>
                        </tr>
                        <tr>
                            <td>
                            AC Operating Voltage (V)
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                            <td>
                                110~220 (±5%）
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Operating Temperature (℃)
                            </td>
                            <td>
                            ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                            ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                            ﹣20~﹢60
                            </td>
                        </tr>
                        <tr>
                            <td>
                            IP Rating (Front/Rear)
                            </td>
                            <td>
                            IP22
                            </td>
                            <td>
                            IP22
                            </td>
                            <td>
                            IP22
                            </td>
                            <td>
                            IP22
                            </td>
                            <td>
                            IP65
                            </td>
                            <td>
                            IP65
                            </td>
                            <td>
                            IP65
                            </td>
                        </tr>
                        <tr>
                            <td>
                            LED Lifetime (H)
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
        <div class="contact-banner">
            <img src="https://www.cinstar-led.com/images/home-new/contact-banner.webp" alt="Contact Us Now - Cinstar LED">
            <div class="contact-banner-content">
                <div class="contact-banner-dialog">
                    <p>Wanna Upgrade Your Next Projects?</p>
                </div>
                <div class="contact-banner-mark">
                    <a href="https://www.cinstar-led.com/contact/" class="contact-banner-button">Contact Us Now</a>
                </div>
            </div>
        </div>
        <?php include('../inc/foot.php'); ?>
            <!-- 底部结束 -->
    </div>
    <?php include('../inc/js.php'); ?>
    <script>GetCurStyle("products");</script>
</body> 
</html>