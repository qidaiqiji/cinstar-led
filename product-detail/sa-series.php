<!DOCTYPE html>
<html lang="en">
<head>
    <?php include('../inc/meta.php'); ?>
    <?php include('../inc/rem.php'); ?>
    <link rel="stylesheet" href="../css/detail-240819.css">
    <title>Sport Arena LED Display | Stadium LED Advertising | Outdoor LED Advertising Screen | Cinstar-led</title>
    <meta name="description" content="Cinstar SA seires outdoor stadium led banner display is the most cost effective stadium led advertising solution for all outdoor stadium and sport arena to maximize your ROI." />
    <meta name="keywords" content="sport arena led display, stadium led advertising, outdoor led advertising, outdoor stadium led display, stadium led screen, stadium advertising billboard, sports led display, sport arena led screen, led perimeter banner screen" />
</head>
<body class="page-index .menu-open">
    <!-- 头部开始 -->
    <?php include('../inc/top.php'); ?>
    <!-- 头部结束 -->
    <div class="container">
        <div class="detail-title sa-bg bg">
            <div class="title">
                <span>SA SERIES</span>
            </div>
            <div class="top-title">Cost-Effective Sport Arena Video Solution</div>
        </div>
  
        <!-- 产品参数 -->
        <div class="specification">
            <div class="spec-detail">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td colspan="2">Specifications</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm):
                            </td>
                            <td>
                            4 / 5 / 6  / 8 / 10
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nits):
                            </td>
                            <td>
                            >6500
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Refresh Rate(Hz):    
                            </td>
                            <td>
                            &lt;3840
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Application: 
                            </td>
                            <td>
                            Sports Arena Advertising
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div class="key-feature">
                    <div>Key Features</div>
                    <ul>
                        <li>Solid LED perimeter banner solution</li>
                        <li>Soft module mask to prevent injury</li>
                        <li>Adjustable supporting brackets</li>
                        <li>Great stability & durability</li>
                        <li>Multifunction for sports events</li>
                        <li>Comprehensive image solution</li>
                    </ul>
                </div>
            </div>
            <div class="product-img small-long">
                <img src="../images/products/Commercial-LED/SA-series-1.jpg" alt="SA SERIES"/>
            </div>
        </div>
        <!-- 产品详情 -->
        <div class="product-detail">
            <div class="detail">
                <div class="img-part half">
                    <img src="../images/products/Commercial-LED/SA-series-2.jpg" alt="SA SERIES"/>
                </div>
                <div class="desc-part half">
                    <h2 class="title">Solid LED Perimeter Banner Solution</h2>
                    <p>
                    Cinstar SA series stadium LED banner system is featured with slim and light weight panel for easy installation and transportation. Super High refresh rate 3840Hz and excellent gray scale 16bit ensuring the superb image production.
                    </p>
                </div>
            </div>
            <hr class="under-line"/>
            <div class="detail">
                <div class="desc-part half">
                    <h2 class="title">Soft Module Mask To Prevent Injury</h2>
                    <p>
                    Cinstar SA series stadium perimeter banner system is using soft rubber module mask to protect LEDs from ball punching. What more important is that the soft module mask can effectively protect athletes from severe injury when collision to LED system by accident.
                    </p>
                </div>
                <div class="img-part half">
                    <img src="../images/products/Commercial-LED/SA-series-3.jpg" alt="SA SERIES"/>
                </div>
            </div>
            <hr class="under-line"/>
            <div class="detail">
                <div class="img-part half">
                    <img src="../images/products/Commercial-LED/SA-series-4.jpg" alt="SA SERIES"/>
                </div>
                <div class="desc-part half">
                    <h2 class="title">Adjustable Supporting Brackets</h2>
                    <p>
                    Cinstar SA series outdoor stadium perimeter banner system adopted flexible supporting brackets which can be adjusted to different lean angles to meet the best shooting angle for cameras and spectators. Which helps to maximize its business value and brings more advertising revenue.
                    </p>
                </div>
            </div>
            <hr class="under-line"/>
            <div class="detail">
                <div class="desc-part half">
                    <h2 class="title">Great Stability & Durability</h2>
                    <p>
                    Cinstar SA series stadium perimeter banner system is featured with high level IP rating and supreme cooling design to guarantee the best reliability and performance. All the quality components and user friendly design enable it the best solution to maximize your ROI. 
                    </p>
                </div>
                <div class="img-part half">
                    <img src="../images/products/Commercial-LED/SA-series-5.jpg" alt="SA SERIES"/>
                </div>
            </div>
            <hr class="under-line"/>
            <div class="detail">
                <div class="img-part half">
                    <img src="../images/products/Commercial-LED/SA-series-6.jpg" alt="SA SERIES"/>
                </div>
                <div class="desc-part half">
                    <h2 class="title">Multifunction For Sports Events</h2>
                    <p>
                    Except used as perimeter banner system, Cinstar SA series sports arena LED system can be installed as scoreboards, live feed video screen or served as advertising billboard by folding up the bracket and get rid of the panel top cushion. It is an <a href="https://www.cinstar-led.com/products/stadium-led-screen.php"> ideal video display solution for stadium</a>, sport arena and horse racing etc.
                    </p>
                </div>
            </div>
            <hr class="under-line"/>
            <div class="detail">
                <div class="desc-part half">
                    <h2 class="title">Comprehensive Image Solution</h2>
                    <p>
                    Normally perimeter display is a long banner with horizontal pixels over the limit of monitor 3840pixels, the whole perimeter banner will be divided into several batches to show same content. But we have solution to make the screen display a complete image even if the pixels more than 30k or even more.
                    </p>
                </div>
                <div class="img-part half">
                    <img src="../images/products/Commercial-LED/SA-series-7.jpg" alt="SA SERIES"/>
                </div>
            </div>
        </div>
        <!-- SERIES FEATURES -->
        <div class="features">
            <div class="sub-title">
                <span>SA SERIES Features</span>
            </div>
            <div class="iconsList">
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-check"></i>
                    </div>
                    <p class="icon-desc">Solid LED perimeter banner solution</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-bars"></i>
                    </div>
                    <p class="icon-desc">Soft module mask to prevent injury</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe621;</i>
                    </div>
                    <p class="icon-desc">Adjustable supporting brackets</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-tint"></i>
                    </div>
                    <p class="icon-desc">Great stability & durability</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe600;</i>
                    </div>
                    <p class="icon-desc">Multifunction for sports events</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-random"></i>
                    </div>
                    <p class="icon-desc">Comprehensive image solution</p>
                </div>

            </div>
        </div>
        <!-- showCases -->
        <div class="show-cases">
            <div class="sub-title">
                <span>SA SERIES Showcase</span>
            </div>
            <div class="img-wrap">
                <div class="caseItem">
                    <img src="../images/products/Commercial-LED/SA-series-8.jpg" alt="Cinstar 8mm Outdoor Stadium Perimeter Display"/>
                    <a class="case-title">
                        <span class="current">Cinstar 8mm Outdoor Stadium Perimeter Display</span>
                    </a>
                </div>
                <div class="caseItem">
                    <img src="../images/products/Commercial-LED/SA-series-9.jpg" alt="Cinstar 10mm Football Stadium Perimeter Screen"/>
                    <a class="case-title">
                        <span class="current">Cinstar 10mm Football Stadium Perimeter Screen</span>
                    </a>
                </div>
            </div>
        </div>
        <div class="table-detail">
            <div class="sub-title">
                <span>Technical Specification</span>
            </div>
            <div class="table-wrap">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td></td>
                            <td>SA4</td>
                            <td>SA5</td>
                            <td>SA6</td>
                            <td>SA8</td>
                            <td>SA10</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm)
                            </td>
                            <td>
                                4
                            </td>
                            <td>
                                5
                            </td>
                            <td>
                                6
                            </td>
                            <td>
                                8
                            </td>
                            <td>
                                10
                            </td>
                        </tr>
                        <tr>
                            <td>
                                LED Type
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Module Size (WxH)/(mm)
                            </td>
                            <td>
                            256x128
                            </td>
                            <td>
                            160x160
                            </td>
                            <td>
                            192x192
                            </td>
                            <td>
                            256x128
                            </td>
                            <td>
                            320x160
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Size (WxH)/(mm)
                            </td>
                            <td>
                            768x768
                            </td>
                            <td>
                            640x640
                            </td>
                            <td>
                            960x768
                            </td>
                            <td>
                            1024x768
                            </td>
                            <td>
                            960x960
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Per Panel
                            </td>
                            <td>
                            192x192
                            </td>
                            <td>
                            128x128
                            </td>
                            <td>
                            160x128
                            </td>
                            <td>
                            128x96
                            </td>
                            <td>
                            96x96
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Panel Material
                            </td>
                            <td>
                                Aluminum
                            </td>
                            <td>
                                Aluminum
                            </td>
                            <td>
                                Aluminum
                            </td>
                            <td>
                                Aluminum
                            </td>
                            <td>
                                Aluminum
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Brightness (nit)
                            </td>
                            <td>
                            > 6500
                            </td>
                            <td>
                            > 5500
                            </td>
                            <td>
                            > 6500
                            </td>
                            <td>
                            > 6500
                            </td>
                            <td>
                            > 6500
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Refresh Rate (Hz)
                            </td>
                            <td>
                                > 1920
                            </td>
                            <td>
                                > 1920
                            </td>
                            <td>
                                > 1920
                            </td>
                            <td>
                            > 1920
                            </td>
                            <td>
                            > 1920
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Gray Scale (bit) 
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Viewing Angle (H/V) (°) 
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                140/140
                            </td>
                            <td>
                                140/140
                            </td>
                            <td>
                                140/140
                            </td>
                            <td>
                                140/140
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Operating Temperature (℃)
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                            ﹣20~﹢60
                            </td>
                            <td>
                            ﹣20~﹢60
                            </td>
                        </tr>
                        <tr>
                            <td>
                            IP Rating (Front/Rear)
                            </td>
                            <td>
                            IP65
                            </td>
                            <td>
                                IP65
                            </td>
                            <td>
                                IP65
                            </td>
                            <td>
                            IP65
                            </td>
                            <td>
                            IP65
                            </td>
                        </tr>
                        <tr>
                            <td>
                            LED Lifetime (H)
                            </td>
                            <td>
                            100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                            100000
                            </td>
                            <td>
                            100000
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Maintenance Method
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
        <div class="related">
            <div class="sub-title">
                <span>Related Products</span>
            </div>
            <div class="product-container">
                <div class="product-item">
                    <a class="thumb" href="../product-detail/bt-series.php">
                        <img src="../images/products/Commercial-LED/BT-series-1.jpg" class="short" alt="bt-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/bt-series.php">
                        <span class="current">BT SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/ma-series.php">
                        <img src="../images/products/Commercial-LED/MA-series-1.jpg" class="short" alt="MA SERIES"/>
                    </a>
                    <a class="list-title" href="../product-detail/ma-series.php">
                        <span class="current">MA SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/ft-series.php">
                        <img src="../images/home/FT SERIES.jpg" class="short" alt="FT SERIES"/>
                    </a>
                    <a class="list-title" href="../product-detail/ft-series.php">
                        <span class="current">FT SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/db-series.php">
                        <img src="../images/home/DB Series.jpg" class="shorter" alt="DB SERIES"/>
                    </a>
                    <a class="list-title" href="../product-detail/db-series.php">
                        <span class="current">DB SERIES</span><span class="more">View More</span>
                    </a>
                </div>
            </div>
        </div>
        <?php include('../inc/foot.php'); ?>
            <!-- 底部结束 -->
    </div>
    <?php include('../inc/js.php'); ?>
    <script>GetCurStyle("products");</script>
</body> 
</html>