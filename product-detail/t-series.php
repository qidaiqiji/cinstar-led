<!DOCTYPE html>
<html lang="en">
<head>
    <?php include('../inc/meta.php'); ?>
    <?php include('../inc/rem.php'); ?>
    <link rel="stylesheet" href="../css/detail-240814.css">
    <title>Concert LED Screen | Cinstar-led, LED Panel Screen | Best LED Video Screen</title>
    <meta name="description" content="Cinstar concert LED screen T Series equipped with a fast locking system for rapid construction, a curving system allowing both convex and concave configurations, hanging and ground stacking installation. This LED panel screen will be the best LED video screen for you." />
    <meta name="keywords" content="Concert LED screen, LED panel screen, best LED video screen, LED video wall rental, outdoor rental LED videowall, outdoor LED screen hire, event production LED screen" />
</head>
<body class="page-index .menu-open">
    <!-- 头部开始 -->
    <?php include('../inc/top.php'); ?>
    <!-- 头部结束 -->
    <div class="container">
        <div class="detail-title vx-bg bg">
            <div class="title">
                <span>T SERIES</span>
            </div>
            <div class="top-title">Cost-Effective Rental LED Display Solutions</div>
        </div>
   
        <!-- 产品参数 -->
        <div class="specification">
            <div class="spec-detail">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td colspan="2">Specifications</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm):
                            </td>
                            <td>
                                1.9 / 2.6 / 2.9 / 3.9 / 4.8
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nits):
                            </td>
                            <td>
                                up to 5500
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Refresh Rate(Hz):    
                            </td>
                            <td>
                                1920-3840
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Application: 
                            </td>
                            <td>
                                Fixed/Rental
                            </td>
                        </tr>
                    </tbody>
                </table>
                <div class="key-feature">
                    <div>Key Features</div>
                    <ul>
                        <li>High standard CNC ensures seamless screen connection</li>
                        <li>Ultra  thin and light weight panel design</li>
                        <li>Quick changeable rear control unit</li>
                        <li>Ergonomically designed handle for convenient manipulation</li>
                        <li>Fast and reliable lock system enables quick installation and dismantle </li>
                        <li>Various installations include suspension, ground stacking or curved</li>    
                    </ul>
                </div>
            </div>
            <div class="product-img">
                <img src="../images/products/Rental-LED/t-series.jpg" alt="T-SERIES"/>
            </div>
        </div>
        <!-- 产品详情 -->
        <div class="product-desc">
            <div class="left-img">
                <img src="../images/products/Rental-LED/VX-Case-Show.jpg" alt="T-Case-Show"/>
            </div>
            <div class="right-desc text-long">
                <h2 class="title">User-Friendly LED Video Displays</h2>
                <p>Aim to supply the most cost-effective rental LED video display solution for indoor and outdoor rental events applications, Cinstar T series utilizes high standard components selection to achieve high grey scale and refresh rate thereby making sure the ultimate image quality on Cinstar rental LED video display.</p>
                <p>T series LED video display was designed with a maximum focus on practicability and reliability. The modular LED video panels are built around a robust and light weight die-casting aluminum frame, easy to operate and maintain. </p>  
                <p>With inbuilt feet and edge protectors, physical damage during transportation and assembly can be largely avoided and greatly improve its stability.</p> 
                <p>T series LED video display comes equipped with a classic fast locking system for rapid installation and dismantle, a flexible curving system allowing creative convex and concave set ups, suspension and floor stacking installation and convenient module front access service.</p>
                <p>Cinstar T series LED video display is widely used at car show, wedding ceremony, fashion show, corporate exhibition and award ceremony to deliver ultimate visual impact.</p>
            </div>
        </div>
        <!-- SERIES FEATURES -->
        <div class="features">
            <div class="sub-title">
                <span>T Series Features</span>
            </div>
            <div class="iconsList">
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-lock"></i>
                    </div>
                    <p class="icon-desc">High precision CNC ensures seamless screen connection</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe601;</i>
                    </div>
                    <p class="icon-desc">Ultra  thin and light weight panel design</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-flash"></i>
                    </div>
                    <p class="icon-desc">Quick changeable rear control unit</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-thumbs-up"></i>
                    </div>
                    <p class="icon-desc">Ergonomically designed handle for convenient manipulation</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="iconfont">&#xe600;</i>
                    </div>
                    <p class="icon-desc">Fast and durable lock system enables quick assemble and dismantle</p>
                </div>
                <div class="iconItems">
                    <div class="iconCricle">
                        <i class="fa fa-check"></i>
                    </div>
                    <p class="icon-desc">Various installations include suspension, ground stacking or curved</p>
                </div>

            </div>
        </div>
        <!-- showCases -->
        <div class="show-cases">
            <div class="sub-title">
                <span>T Series Showcase</span>
            </div>
            <div class="img-wrap">
                <div class="caseItem">
                    <img src="../images/rental/16-rental-case.jpg" alt="CR SERIES SHOWCASE"/>
                    <a class="case-title">
                        <span class="current">UK Campbell Park T4.8 Outdoor LED Screen Display</span>
                    </a>
                </div>
                <div class="caseItem">
                    <img src="../images/rental/18-rental-case.jpg" alt="CR SERIES SHOWCASE"/>
                    <a class="case-title">
                        <span class="current">North Ireland UUP Conference Party T3.9 LED Screen Display</span>
                    </a>
                </div>
            </div>
        </div>
        <div class="table-detail">
            <div class="sub-title">
                <span>Technical Specification</span>
            </div>
            <div class="table-wrap">
                <table>
                    <tbody>
                        <tr class="thead">
                            <td></td>
                            <td>T2.6</td>
                            <td>T2.9</td>
                            <td>T3.9</td>
                            <td>T2.9(Outdoor)</td>
                            <td>T3.9(Outdoor)</td>
                        </tr>
                        <tr>
                            <td>
                                Pixel Pitch (mm)
                            </td>
                            <td>
                            2.6
                            </td>
                            <td>
                            2.97
                            </td>
                            <td>
                            3.91
                            </td>
                            <td>
                            2.97
                            </td>
                            <td>
                            3.91
                            </td>
                        </tr>
                        <tr>
                            <td>
                                LED Type
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                                3 in 1 SMD
                            </td>
                            <td>
                            3 in 1 SMD
                            </td>
                            <td>
                            3 in 1 SMD
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Module Size (WxH)/(mm)
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                            <td>
                                250x250
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Size (WxH)/(mm)
                            </td>
                            <td>
                                500x500 / 500x1000
                            </td>
                            <td>
                                500x500 / 500x1000
                            </td>
                            <td>
                                500x500 / 500x1000
                            </td>
                            <td>
                                500x500 / 500x1000
                            </td>
                            <td>
                                500x500 / 500x1000
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Pixel Per Panel
                            </td>
                            <td>
                            192x192 / 192x384
                            </td>
                            <td>
                            168x168 / 168x336
                            </td>
                            <td>
                            128x128 / 128x256
                            </td>
                            <td>
                            168x168 / 168x336
                            </td>
                            <td>
                            128x128 / 128x256
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Panel Material
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                            <td>
                                Die-casting Aluminum
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Panel Weight (kg)
                            </td>
                            <td>
                            7.5 / 14
                            </td>
                            <td>
                            7.5 / 14
                            </td>
                            <td>
                            7.5 / 14
                            </td>
                            <td>
                            7.8 / 14.5
                            </td>
                            <td>
                            7.8 / 14.5
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Brightness (nit)
                            </td>
                            <td>
                                > 1000
                            </td>
                            <td>
                                > 1000
                            </td>
                            <td>
                            > 1200
                            </td>
                            <td>
                            > 5500
                            </td>
                            <td>
                            > 5500
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Refresh Rate (Hz)
                            </td>
                            <td>
                            1920-3840
                            </td>
                            <td>
                            1920-3840
                            </td>
                            <td>
                            1920-3840
                            </td>
                            <td>
                            1920-3840
                            </td>
                            <td>
                            1920-3840
                            </td>
                        </tr>
                        <tr>
                            <td>
                                Gray Scale (bit) 
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                            <td>
                                16
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Viewing Angle (H/V) (°) 
                            </td>
                            <td>
                                160/160
                            </td>
                            <td>
                                160/160
                            </td>
                            <td>
                                160/160
                            </td>
                            <td>
                            160/140
                            </td>
                            <td>
                            160/140
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Avg. Power Consumption (W)
                            </td>
                            <td>
                                240/sqm
                            </td>
                            <td>
                            210/sqm
                            </td>
                            <td>
                            218/sqm
                            </td>
                            <td>
                            236/sqm
                            </td>
                            <td>
                            243/sqm
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Max. Power Consumption (W)
                            </td>
                            <td>
                            720/sqm
                            </td>
                            <td>
                            630/sqm
                            </td>
                            <td>
                            654/sqm
                            </td>
                            <td>
                            708/sqm
                            </td>
                            <td>
                            729/sqm
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Operating Temperature (℃)
                            </td>
                            <td>
                            ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                                ﹣20~﹢60
                            </td>
                            <td>
                            ﹣20~﹢60
                            </td>
                            <td>
                            ﹣20~﹢60
                            </td>
                        </tr>
                        <tr>
                            <td>
                            IP Rating (Front/Rear)
                            </td>
                            <td>
                            IP40/IP21
                            </td>
                            <td>
                                IP40/IP21
                            </td>
                            <td>
                                IP40/IP21
                            </td>
                            <td>
                                IP65
                            </td>
                            <td>
                                IP65
                            </td>
                        </tr>
                        <tr>
                            <td>
                            LED Lifetime (H)
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                            <td>
                                100000
                            </td>
                        </tr>
                        <tr>
                            <td>
                            Module Maintenance 
                            </td>
                            <td>
                                Front/Rear
                            </td>
                            <td>
                                Front/Rear
                            </td>
                            <td>
                                Front/Rear
                            </td>
                            <td>
                                Front/Rear
                            </td>
                            <td>
                                Front/Rear
                            </td>
                        </tr>
                        <tr>
                            <td>
                            PSU & Others Maintenance
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                            <td>
                                Rear
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
        <div class="related">
            <div class="sub-title">
                <span>Related Products</span>
            </div>
            <div class="product-container">
                <div class="product-item">
                    <a class="thumb" href="../product-detail/cr-series.php">
                        <img src="../images/home/CR SERIES.jpg" alt="cr-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/cr-series.php">
                        <span class="current">CR SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/lm-series.php">
                        <img src="../images/home/LM SERIES.jpg" alt="lm-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/lm-series.php">
                        <span class="current">LM SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/tr-series.php">
                        <img src="../images/home/TR SERIES.jpg" class="short" alt="tr-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/tr-series.php">
                        <span class="current">TR SERIES</span><span class="more">View More</span>
                    </a>
                </div>
                <div class="product-item">
                    <a class="thumb" href="../product-detail/ei-series.php">
                        <img src="../images/home/EI Series.jpg" class="shorter" alt="ei-series"/>
                    </a>
                    <a class="list-title" href="../product-detail/ei-series.php">
                        <span class="current">EI SERIES</span><span class="more">View More</span>
                    </a>
                </div>
            </div>
        </div>
        <?php include('../inc/foot.php'); ?>
            <!-- 底部结束 -->
    </div>
    <?php include('../inc/js.php'); ?>
    <script>GetCurStyle("products");</script>
</body> 
</html>